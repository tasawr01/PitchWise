...
import { isDealAwaitingPayment, isDealPaid } from '@/lib/deal-status';
...
async function getInvestorStats() {
    ...
    try {
        ...
        const totalInvested = deals
            .filter(d => isDealPaid(d))
            .reduce((acc, curr) => acc + (curr.amount || 0), 0);

        const pendingDeals = deals.filter(d => isDealAwaitingPayment(d)).length;
        const completedDeals = deals.filter(d => isDealPaid(d)).length;
